...
from django.db.models import Avg, Count, Sum, Min, Max, Q, IntegerField, FloatField, Case, When, Value, F
...
class tbdb(models.Model):
    ...
    def compress_to_hourly(cls, start_date=None, end_date=None):
        ...
        For each measurement_name, calculate hourly averages and store in tbdb_hourly.
        
        Args:
            start_date: Start date for compression (optional)
            end_date: End date for compression (optional)
        """
        
        # Get all unique measurement names and types
        cursor = connection.cursor()
        table_name = cls._meta.db_table
        
        # First, get all unique measurement names with their types
        sql = f"SELECT DISTINCT measurement_name, measurement_type FROM {table_name}"
        params = []
        ...
        for measurement_name, measurement_type in measurements:
            # Only compress numeric measurements
            if measurement_type not in ['integer', 'float', 'decimal']:
                continue
                
            # Get the date range for this measurement
            date_sql = f"SELECT MIN(timestamp), MAX(timestamp) FROM {table_name} WHERE measurement_name = %s"
            date_params = [measurement_name]
            
            if start_date and end_date:
                date_sql += " AND timestamp BETWEEN %s AND %s"
                date_params.extend([start_date, end_date])
            elif start_date:
                date_sql += " AND timestamp >= %s"
                date_params.append(start_date)
            elif end_date:
                date_sql += " AND timestamp <= %s"
                date_params.append(end_date)
                
            cursor.execute(date_sql, date_params)
            ...
            # Round down to hour for start, round up to hour for end
            current_hour = min_date.replace(minute=0, second=0, microsecond=0)
            end_hour = max_date.replace(minute=0, second=0, microsecond=0) + timedelta(hours=1)
            ...
            while current_hour < end_hour:
                ...
                avg_sql = f"""
                    ...
                    AND timestamp < %s
                """
                cursor.execute(avg_sql, [measurement_name, current_hour, next_hour])
                ...
                if avg_result is not None:
                    ...
                    if not existing:
                        ...
                        if current_hour.tzinfo is None:
                            current_hour = django_timezone.make_aware(current_hour)
                        
                        hourly_record.timestamp = current_hour
                        hourly_record.value = str(avg_result)
                        hourly_record.save()
                        
                        compression_results.append({
                            'measurement_name': measurement_name,
                            'timestamp': current_hour,
                            'average_value': avg_result
                        })
...
class tbdb_hourly(models.Model):
    """
    Hourly compressed version of tbdb containing hourly averages.
    This model has the same structure as tbdb but stores data compressed to hourly intervals.
    """
    timestamp = models.DateTimeField(default=timezone.now)
    measurement_name = models.CharField(max_length=100)
    measurement_type = models.CharField(max_length=20, 
    ...
    def __str__(self):
        return f"tbdb_hourly(name='{self.measurement_name}', type={self.measurement_type}, data={self.data}, time={self.timestamp})"
    ...
    def get_hourly_statistics(cls, measurement_name=None, start_date=None, end_date=None):
        """
        Get statistics for hourly compressed data.
        
        Args:
            measurement_name: Filter by measurement name (optional)
        ...
        """
        measurements = cls.get_measurements_between_dates(start_date, end_date, measurement_name)
        numeric_measurements = [m for m in measurements if m.measurement_type in ['integer', 'float', 'decimal']]
        ...
        data_values = [m.data for m in numeric_measurements]
